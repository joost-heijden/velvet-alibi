...
const run = async () => {
  ...
  await mobile.getByRole('button', { name: /Check dossier/i }).click()
  await assertText(mobile, /botsen met het dossier/i, 'wrong-grid status')
  await mobile.getByRole('button', { name: /Toon hint/i }).click()
  await assertText(mobile, /Begin linksboven/i, 'hint copy')
  await mobile.getByTestId('accuse-ada').click()
  await assertText(mobile, /klopt niet/i, 'wrong accusation')
  await mobile.getByTestId('qa-fill').click()
  await mobile.getByTestId('accuse-mara').click()
  await mobile.getByRole('button', { name: /Deel resultaat/i }).click()
  ...
  await assertText(desktop, /Finale beschuldiging/i, 'desktop accusation panel')
